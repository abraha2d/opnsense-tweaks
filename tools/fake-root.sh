...
if [ -L /var/run ]; then
    if [ ! -d /run/dhcpcd ]; then
        mkdir -p /run/dhcpcd 2>&1 || sudo -n mkdir -p /run/dhcpcd 2>&1 || {
            echo "[fake-root] error: cannot create host mount point /run/dhcpcd" >&2
            exit 1
        }
    fi
    if [ ! -d /var/run/dhcpcd ]; then
        mkdir -p /var/run/dhcpcd 2>&1 || sudo -n mkdir -p /var/run/dhcpcd 2>&1 || {
            echo "[fake-root] error: cannot create host mount point /var/run/dhcpcd" >&2
            exit 1
        }
    fi
fi
if [ ! -d /root/opnsense-tweaks ]; then
    mkdir -p /root/opnsense-tweaks 2>&1 || sudo -n mkdir -p /root/opnsense-tweaks 2>&1 || echo "[fake-root] warn: host /root not writable (550), will skip bind" >&2
fi
